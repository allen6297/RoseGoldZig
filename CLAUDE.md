...
zig build test                  # run every test (241 as of writing)
...
| `examples/*.rg` | Sample programs: `demo.rg` (single file), `app.rg` + `mathutil.rg` + `geometry.rg` (modules), `messy.rg` (badly-formatted input for `fmt`). `tour.repl` is a REPL input script (`repl < examples/tour.repl`). |
